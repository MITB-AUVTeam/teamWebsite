  ...
  useEffect(() => {
    ...
    const moveHandler = (e: MouseEvent) => {
      ...
      if (targetEl && typeof targetEl.closest === 'function') {
        const isOverHighZ = targetEl.closest('header, [class*="z-[10000]"], [class*="z-[20000]"], .z-\\[10000\\], .z-\\[20000\\]') !== null;
        if (cursorRef.current) {
          cursorRef.current.style.zIndex = isOverHighZ ? '999999' : '9999';
        }
      }
    };
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      while (current && current !== document.body) {
        if (current.matches(targetSelector)) {
          const isBackgroundItem = current.closest('.grid__item') !== null && current.closest('.bento-container') === null;
          if (!isBackgroundItem) {
      ...
      activeTarget = target;
      const isHeaderOrModal = target.closest('header, [class*="z-[10000]"], [class*="z-[20000]"]') !== null;
      const isBackgroundItem = target.closest('.grid__item') !== null && target.closest('.bento-container') === null;
      if (cursorRef.current) {
        if (isHeaderOrModal) {
          cursorRef.current.style.zIndex = '999999';
        } else if (isBackgroundItem) {
          cursorRef.current.style.zIndex = '12';
        } else {
